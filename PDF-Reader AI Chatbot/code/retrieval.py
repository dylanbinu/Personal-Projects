...
def retrieve_and_rank(
    ...
    if ("service" in query.lower() and "times" in query.lower()) or \
       ...
       ("visit" in query.lower()):
        ...
        for url in location_urls:
             ...
             f = build_filter(url)
             hits = vector_db.similarity_search("service times", k=2, filter=f)
    ...
    if any(q in query.lower() for q in ["give", "giving", "donate", "tithe"]):
        ...
        for url in giving_urls:
             f = build_filter(url)
             hits = vector_db.similarity_search("giving", k=2, filter=f)
             forced_docs.extend(hits)
    ...
    if any(q in query.lower() for q in ["pastor", "team", "staff", "leader", "who"]):
        ...
        for url in team_urls:
             f = build_filter(url)
             hits = vector_db.similarity_search("pastors staff team", k=2, filter=f)
             forced_docs.extend(hits)
    ...
    if any(q in query.lower() for q in ["youth", "kid", "child", "student", "teen"]):
        ...
        for url in ministry_urls:
             f = build_filter(url)
             hits = vector_db.similarity_search("youth kids ministry", k=2, filter=f)
             forced_docs.extend(hits)
